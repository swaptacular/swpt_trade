...
Revision ID: f3c9eea538fd
Revises: 286d0ee4d117
Create Date: 2024-02-13 21:56:25.292296
...
def upgrade_():
    ...
    sa.Column('locator_claimed_at', sa.TIMESTAMP(timezone=True), nullable=True),
    sa.Column('latest_debtor_info_fetch_at', sa.TIMESTAMP(timezone=True), nullable=True),
    sa.CheckConstraint('debtor_info_locator IS NULL OR locator_claimed_at IS NOT NULL AND latest_debtor_info_fetch_at IS NOT NULL'),
    sa.PrimaryKeyConstraint('debtor_id')
    )
    with op.batch_alter_table('anchor_debtor', schema=None) as batch_op:
        batch_op.create_index('idx_anchor_debtor_fetch_at', ['latest_debtor_info_fetch_at'], unique=False)
    ...
    op.create_table('locate_debtor_signal',
    sa.Column('signal_id', sa.BigInteger(), autoincrement=True, nullable=False),
    sa.Column('debtor_id', sa.BigInteger(), nullable=False),
    sa.Column('iri', sa.String(), nullable=False),
    sa.Column('inserted_at', sa.TIMESTAMP(timezone=True), nullable=False),
...
def downgrade_():
    ...
    op.drop_table('locate_debtor_signal')
    ...
    with op.batch_alter_table('anchor_debtor', schema=None) as batch_op:
        batch_op.drop_index('idx_anchor_debtor_fetch_at')
